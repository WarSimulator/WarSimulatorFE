...
import type { DeploymentUnit, TacticalGraphic } from '../../../types';
...
type Props<T> = { map: MapLibreMap; graphic: T;
  onPreview: (graphic: T) => void; onCommit: (graphic: T) => void };

export function GraphicRotationHandle(props: Props<TacticalGraphic>) {
  return <RotationHandle {...props} coordinates={graphicCoordinates} centerOf={rotationCenter} rotate={rotateGraphic} offset={34} label="Rotate graphic" />;
}

const unitCoordinates = (unit: DeploymentUnit) => [getLngLat(unit.position)];
const unitCenter = (unit: DeploymentUnit) => getLngLat(unit.position);
const rotateUnit = (unit: DeploymentUnit, angle: number): DeploymentUnit => ({
  ...unit, symbolRotation: (((unit.symbolRotation ?? 0) + angle * 180 / Math.PI) % 360 + 360) % 360,
});
export function UnitRotationHandle(props: Props<DeploymentUnit>) {
  return <RotationHandle {...props} coordinates={unitCoordinates} centerOf={unitCenter} rotate={rotateUnit}
    offset={60 * (props.graphic.symbolScale ?? 1) + 20} label="Rotate military symbol" />;
}

function RotationHandle<T>({ map, graphic, onPreview, onCommit, coordinates, centerOf, rotate, offset, label }: Props<T> & {
  coordinates: (value: T) => [number, number][];
  centerOf: (value: T) => [number, number];
  rotate: (value: T, angle: number, center: [number, number]) => T;
  offset: number; label: string;
}) {
  const [position, setPosition] = useState<{ x: number; y: number } | null>(null);
  const [degrees, setDegrees] = useState<number | null>(null);
  const drag = useRef<{ base: T; center: [number, number]; start: number; latest: T; moved: boolean } | null>(null);
  ...
  useEffect(() => {
    const update = () => {
      ...
      const points = coordinates(graphic).map(p => map.project(p));
      const x = (Math.min(...points.map(p => p.x)) + Math.max(...points.map(p => p.x))) / 2;
      const y = Math.min(...points.map(p => p.y)) - offset;
      setPosition({ x, y: Math.max(20, y) });
    };
    update(); map.on('move', update);
    return () => { map.off('move', update); };
  }, [map, graphic, coordinates, offset]);
  ...
  return <button type="button" aria-label={label} title="드래그하여 회전 · 놓으면 완료 · Esc로 취소"
    ...
    onPointerMove={event => {
      ...
      current.latest = rotate(current.base, angle, current.center);
