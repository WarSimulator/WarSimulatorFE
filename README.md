...
v3 카탈로그는 대표 카드 1,843개와 원본 별칭 804개를 제공합니다.
에디터의 아군·적군 SVG와 제대 지원이 같은 항목은 한 카드로 합치고,
원래 이름·SIDC·표준·분류는 aliases에 보존해 검색과 이전 배치 복원에 사용합니다.
빈 기본 프레임, Disused 및 미검증 통제도형은 검토 대상으로 남깁니다.
표시 보정된 Minefield는 한 카드로 합칩니다. 검토 대상에는 시각적 중복이 남을 수 있습니다.
Minefield(270701)는 상위 렌더러가 빈 사각형만 출력하므로 공통 SVG 렌더러에서
사용자 제공 이미지에 맞춘 세로 사각형·빈 원 4개의 표시 보정을 적용합니다.
이는 표준 전체의 적합성 검증과는 별개입니다.
...
팔레트 필터 아래 `DRAW + 전술 과업` 버튼에서 MIL-STD-2525E 도형을 선택합니다.
심볼 / DRAW + 전술 과업 버튼으로 목록을 전환합니다. 기본 DRAW 6종과 전술 과업은 모양 미리보기 카드로 표시합니다. 상단 검색·카테고리로 과업을 찾고, 카드를 누른 뒤 미리보기 번호 순서대로 지도에 클릭합니다. 심볼 목록의 검색과 카테고리는 전환 후에도 유지됩니다.
