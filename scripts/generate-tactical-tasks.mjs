...
import { MSLookup, WebRenderer, ErrorLogger, MilStdIconRenderer, MilStdAttributes } from '@armyc2.c5isr.renderer/mil-sym-ts-web';
const korean = {
  '151204':'봉쇄', '151205':'유지', '340900':'격멸', '341400':'차단 공격', '341600':'무력화', '342800':'제압', '344300':'격퇴',
...
for (const [version, standard] of [[13,'2525E']]) {
 for(const id of lookup.getIDList(version)) {
  ...
  if(info.getMinPointCount() === 1) {
    // Pre-generate the unsupported Defeat icon from the upstream SVG renderer.
    // It does not use text measurement for this icon; Node canvas init is unused.
    let svg;
    if(!new ms.Symbol(sidc).isValid()) svg=MilStdIconRenderer.getInstance().RenderSVG(sidc,new Map(),new Map([[MilStdAttributes.PixelSize,'64']])).getSVG();
    tasks.push({id:`${standard}:${entity}`,sidc,standard,label:info.getName(),korean:korean[entity]??info.getName(),minPoints:1,maxPoints:1,drawRule:info.getDrawRule(),samplePoints,preview:[],pointSymbol:true,...(svg ? {svg} : {})});
    continue;
  }
  rendererErrors=[];
